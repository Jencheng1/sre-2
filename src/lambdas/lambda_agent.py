"""
Lambda Monitoring Agent for AWS Bedrock

This module implements a specialized AWS Bedrock agent for monitoring AWS Lambda functions.
It detects issues related to function performance, errors, cold starts, and resource utilization.
"""

import json
import boto3
import time
import uuid
from datetime import datetime, timedelta
from typing import Dict, List, Any, Optional

from .mcp import MCPMessageFactory
from .a2a import A2AAgent, A2AMessageBroker


class LambdaMonitoringAgent(A2AAgent):
    """Agent for monitoring AWS Lambda functions and detecting issues."""
    
    def __init__(self, agent_id: str, broker: A2AMessageBroker, 
                 supervisor_id: str, region_name: str = 'us-east-1'):
        """Initialize the Lambda monitoring agent."""
        super().__init__(agent_id, broker)
        self.supervisor_id = supervisor_id
        self.region_name = region_name
        self.lambda_client = boto3.client('lambda', region_name=region_name)
        self.cloudwatch = boto3.client('cloudwatch', region_name=region_name)
        self.bedrock_runtime = boto3.client('bedrock-runtime', region_name=region_name)
        
        # Register capabilities with supervisor
        self.register_with_supervisor()
        
        # Define metrics to monitor
        self.metrics = [
            "Invocations",
            "Errors",
            "Duration",
            "Throttles",
            "ConcurrentExecutions",
            "UnreservedConcurrentExecutions",
            "ProvisionedConcurrentExecutions",
            "IteratorAge",
            "DeadLetterErrors"
        ]
        
        # Track recent alerts to avoid duplicates
        self.recent_alerts = {}
    
    def register_with_supervisor(self) -> None:
        """Register this agent with the supervisor."""
        capabilities = [
            "lambda_monitoring",
            "function_metrics",
            "error_analysis",
            "cold_start_detection"
        ]
        
        registration_data = {
            "agent_type": "monitoring",
            "capabilities": capabilities,
            "monitored_service": "Lambda"
        }
        
        self.send_query(
            recipient_id=self.supervisor_id,
            content=json.dumps(registration_data),
            query_type="register"
        )
    
    def get_functions(self) -> List[Dict[str, Any]]:
        """Get list of Lambda functions."""
        try:
            functions = []
            paginator = self.lambda_client.get_paginator('list_functions')
            
            for page in paginator.paginate():
                functions.extend(page.get('Functions', []))
            
            return functions
        except Exception as e:
            print(f"Error retrieving Lambda functions: {e}")
            self.send_notification(
                content=f"Error retrieving Lambda functions: {e}",
                notification_type="monitoring_error",
                severity="error",
                recipient_id=self.supervisor_id
            )
            return []
    
    def get_function_metrics(self, function_name: str) -> Dict[str, List[Dict[str, Any]]]:
        """Get CloudWatch metrics for a Lambda function."""
        try:
            end_time = datetime.utcnow()
            start_time = end_time - timedelta(minutes=5)
            
            metric_data = {}
            for metric in self.metrics:
                response = self.cloudwatch.get_metric_statistics(
                    Namespace='AWS/Lambda',
                    MetricName=metric,
                    Dimensions=[{'Name': 'FunctionName', 'Value': function_name}],
                    StartTime=start_time,
                    EndTime=end_time,
                    Period=300,
                    Statistics=['Average', 'Maximum', 'Sum']
                )
                metric_data[metric] = response.get('Datapoints', [])
            
            return metric_data
        except Exception as e:
            print(f"Error retrieving metrics for function {function_name}: {e}")
            return {}
    
    def get_function_config(self, function_name: str) -> Dict[str, Any]:
        """Get Lambda function configuration."""
        try:
            response = self.lambda_client.get_function_configuration(
                FunctionName=function_name
            )
            return response
        except Exception as e:
            print(f"Error retrieving config for function {function_name}: {e}")
            return {}
    
    def analyze_function_health(self, function_name: str) -> List[Dict[str, Any]]:
        """Analyze the health of a Lambda function."""
        issues = []
        
        try:
            # Get function configuration
            config = self.get_function_config(function_name)
            if not config:
                return issues
            
            # Check memory and timeout settings
            memory = config.get('MemorySize', 128)
            timeout = config.get('Timeout', 3)
            
            if memory < 256:
                issue = {
                    'id': str(uuid.uuid4()),
                    'timestamp': datetime.utcnow().isoformat(),
                    'function_name': function_name,
                    'severity': 'low',
                    'issue_type': 'low_memory',
                    'description': f"Lambda function {function_name} has low memory allocation: {memory}MB"
                }
                issues.append(issue)
            
            if timeout < 10:
                issue = {
                    'id': str(uuid.uuid4()),
                    'timestamp': datetime.utcnow().isoformat(),
                    'function_name': function_name,
                    'severity': 'low',
                    'issue_type': 'low_timeout',
                    'description': f"Lambda function {function_name} has low timeout: {timeout}s"
                }
                issues.append(issue)
            
            # Check metrics
            metrics = self.get_function_metrics(function_name)
            for metric, datapoints in metrics.items():
                if not datapoints:
                    continue
                
                latest = max(datapoints, key=lambda x: x['Timestamp'])
                average = latest.get('Average', 0)
                maximum = latest.get('Maximum', 0)
                total = latest.get('Sum', 0)
                
                if metric == 'Errors' and total > 0:
                    issue = {
                        'id': str(uuid.uuid4()),
                        'timestamp': datetime.utcnow().isoformat(),
                        'function_name': function_name,
                        'metric': metric,
                        'value': total,
                        'threshold': 0,
                        'severity': 'high',
                        'issue_type': 'function_errors',
                        'description': f"Lambda function {function_name} has errors: {total}"
                    }
                    issues.append(issue)
                
                elif metric == 'Throttles' and total > 0:
                    issue = {
                        'id': str(uuid.uuid4()),
                        'timestamp': datetime.utcnow().isoformat(),
                        'function_name': function_name,
                        'metric': metric,
                        'value': total,
                        'threshold': 0,
                        'severity': 'high',
                        'issue_type': 'function_throttles',
                        'description': f"Lambda function {function_name} is being throttled: {total} times"
                    }
                    issues.append(issue)
                
                elif metric == 'Duration' and average > (timeout * 1000 * 0.8):  # 80% of timeout
                    issue = {
                        'id': str(uuid.uuid4()),
                        'timestamp': datetime.utcnow().isoformat(),
                        'function_name': function_name,
                        'metric': metric,
                        'value': average,
                        'threshold': timeout * 1000 * 0.8,
                        'severity': 'medium',
                        'issue_type': 'high_duration',
                        'description': f"Lambda function {function_name} has high duration: {average}ms"
                    }
                    issues.append(issue)
                
                elif metric == 'IteratorAge' and maximum > 300000:  # 5 minutes
                    issue = {
                        'id': str(uuid.uuid4()),
                        'timestamp': datetime.utcnow().isoformat(),
                        'function_name': function_name,
                        'metric': metric,
                        'value': maximum,
                        'threshold': 300000,
                        'severity': 'high',
                        'issue_type': 'high_iterator_age',
                        'description': f"Lambda function {function_name} has high iterator age: {maximum}ms"
                    }
                    issues.append(issue)
                
                elif metric == 'DeadLetterErrors' and total > 0:
                    issue = {
                        'id': str(uuid.uuid4()),
                        'timestamp': datetime.utcnow().isoformat(),
                        'function_name': function_name,
                        'metric': metric,
                        'value': total,
                        'threshold': 0,
                        'severity': 'high',
                        'issue_type': 'dead_letter_errors',
                        'description': f"Lambda function {function_name} has dead letter errors: {total}"
                    }
                    issues.append(issue)
            
            return issues
        
        except Exception as e:
            print(f"Error analyzing function health for {function_name}: {e}")
            return []
    
    def analyze_with_bedrock(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Use AWS Bedrock to analyze Lambda issues."""
        if not issues:
            return {"analysis": "No issues to analyze"}
        
        # Prepare issues for analysis (limit to 10 for prompt size)
        issues_text = "\n".join([
            f"Type: {issue.get('issue_type')} | Function: {issue.get('function_name')} | " +
            f"Severity: {issue.get('severity')} | Description: {issue.get('description')}"
            for issue in issues[:10]
        ])
        
        # Prepare prompt for Claude 3 Haiku
        prompt = f"""
        Analyze the following AWS Lambda issues and provide insights:
        
        {issues_text}
        
        Please identify:
        1. Critical issues requiring immediate attention
        2. Potential impact on application performance and reliability
        3. Root causes and patterns
        4. Recommended remediation steps
        
        Format your response as JSON with the following structure:
        {{
            "critical_issues": [list of issues with reasons],
            "potential_impacts": [list of impacts],
            "root_causes": [list of root causes],
            "remediation_steps": [list of steps]
        }}
        """
        
        try:
            # Call Claude 3 Haiku via Bedrock
            response = self.bedrock_runtime.invoke_model(
                modelId='anthropic.claude-3-haiku-20240307-v1:0',
                contentType='application/json',
                accept='application/json',
                body=json.dumps({
                    "anthropic_version": "bedrock-2023-05-31",
                    "max_tokens": 1000,
                    "messages": [
                        {
                            "role": "user",
                            "content": prompt
                        }
                    ]
                })
            )
            
            # Parse the response
            response_body = json.loads(response['body'].read().decode('utf-8'))
            content = response_body['content'][0]['text']
            
            # Extract JSON from the response
            try:
                # Find JSON in the response
                json_start = content.find('{')
                json_end = content.rfind('}') + 1
                if json_start >= 0 and json_end > json_start:
                    json_str = content[json_start:json_end]
                    analysis = json.loads(json_str)
                else:
                    analysis = {"error": "Could not extract JSON from response"}
            except json.JSONDecodeError:
                analysis = {"error": "Invalid JSON in response"}
            
            return analysis
        
        except Exception as e:
            print(f"Error analyzing with Bedrock: {e}")
            return {"error": str(e)}
    
    def report_issues(self, issues: List[Dict[str, Any]], analysis: Dict[str, Any]) -> None:
        """Report detected issues to the supervisor."""
        if not issues:
            return
        
        # Prepare the report
        report = {
            "timestamp": datetime.utcnow().isoformat(),
            "agent_id": self.agent_id,
            "service": "Lambda",
            "issues": issues,
            "analysis": analysis
        }
        
        # Send the report
        self.send_notification(
            content=json.dumps(report),
            notification_type="monitoring_alert",
            severity="high" if any(issue.get('severity') == 'high' for issue in issues) else "medium",
            recipient_id=self.supervisor_id
        )
    
    def handle_query(self, message: MCPMessage) -> Optional[str]:
        """Handle queries from other agents."""
        try:
            data = json.loads(message.content)
            query_type = data.get("query_type")
            
            if query_type == "function_health":
                function_name = data.get("function_name")
                if function_name:
                    issues = self.analyze_function_health(function_name)
                    analysis = self.analyze_with_bedrock(issues)
                    return json.dumps({
                        "function_name": function_name,
                        "issues": issues,
                        "analysis": analysis
                    })
            
            return None
        
        except Exception as e:
            print(f"Error handling query: {e}")
            return None
    
    def monitor_cycle(self) -> None:
        """Run a monitoring cycle to check all functions."""
        try:
            # Get all functions
            functions = self.get_functions()
            
            all_issues = []
            for function in functions:
                function_name = function['FunctionName']
                # Analyze function health
                issues = self.analyze_function_health(function_name)
                all_issues.extend(issues)
            
            # Analyze issues with Bedrock
            analysis = self.analyze_with_bedrock(all_issues)
            
            # Report issues
            self.report_issues(all_issues, analysis)
        
        except Exception as e:
            print(f"Error in monitoring cycle: {e}")
            self.send_notification(
                content=f"Error in Lambda monitoring cycle: {e}",
                notification_type="monitoring_error",
                severity="error",
                recipient_id=self.supervisor_id
            )
    
    def run_monitoring(self, interval: int = 300, max_runtime: Optional[int] = None) -> None:
        """Run continuous monitoring with specified interval."""
        start_time = time.time()
        
        while True:
            try:
                # Run monitoring cycle
                self.monitor_cycle()
                
                # Check if we've exceeded max runtime
                if max_runtime and (time.time() - start_time) > max_runtime:
                    print("Max runtime exceeded, stopping monitoring")
                    break
                
                # Wait for next interval
                time.sleep(interval)
            
            except Exception as e:
                print(f"Error in monitoring loop: {e}")
                time.sleep(60)  # Wait a minute before retrying


def create_lambda_agent(agent_id: str, supervisor_id: str, region_name: str = 'us-east-1') -> LambdaMonitoringAgent:
    """Create a Lambda monitoring agent."""
    broker = A2AMessageBroker()
    return LambdaMonitoringAgent(agent_id, broker, supervisor_id, region_name) 