#!/usr/bin/env python3
"""
Fixed Test Suite for CPU Spike Demo with Payment Integration
...
class TestCPUPaymentDemo:
    ...
    def test_aws_api_connectivity(self):
        ...
        try:
            # Test SSM - use describe_ops_items instead
    ...
    def test_cloudwatch_metrics_capture(self):
        ...
        try:
            # Get instance ID - handle both EC2 and non-EC2 environments
            try:
                ...
                # If not on EC2, use a placeholder
                instance_id = "local-test"
                
            if instance_id != "local-test":
...
def run_all_tests():
    ...
    logger.info("="*60)
    logger.info("CPU PAYMENT DEMO TEST SUITE - FIXED VERSION")
    logger.info("="*60)
    ...
    # Generate report
    report = {
        "test_suite": "CPU Payment Demo Complete Test - Fixed",
        "timestamp": datetime.now().isoformat(),
    ...
    # Save report
    report_file = f"cpu_payment_test_report_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
